...
describe('POST /friend/add/:idOther', () => {
    ...
    it('Cannot send friend request for user, who sent you.', async () => {
        await FriendService.sendFriendRequest(idUser2.toString(), idUser1.toString());
        const response = await request(app)
            .post('/friend/add/' + idUser2)
            .set({ token: token1 });
        const { user, success, message } = response.body;
